...
- **Image Upload**: Users can upload images of their face to be analyzed for skin conditions.
- **Skin Condition Detection**: The app detects multiple skin conditions like acne, blackheads, dark circles, dry skin, and hyperpigmentation using a pre-trained TensorFlow Lite model.
- **Ayurvedic Advice**: For the detected skin conditions, the app generates a warning message with Ayurvedic treatment suggestions (without providing direct solutions) using the **Google Gemini** generative model.
...
- Python 3.10+
- TensorFlow Lite Model (`skin_condition_model.tflite`)
- Google Cloud API Key
...
1. Clone this repository:
    ```bash
    git clone https://github.com/ashutoshbins/skin-condition-detector.git
    cd skin-condition-detector
    ```
...
3. Place the TensorFlow Lite model (`skin_condition_model.tflite`) in the root directory of the project.
...
6. Open the app in your web browser. Upload an image, and the app will analyze the skin condition and provide Ayurvedic advice.

## Usage

- **Upload an Image**: Click on the "Choose an image..." button to upload a photo.
- **Prediction Results**: The app will show the top detected skin conditions along with their probabilities.
- **Ayurvedic Advice**: Based on the top detected condition, a warning message will be generated with Ayurvedic advice (without offering solutions).
